...
test framework.  Version 0.6.0 is a minor bug fix release.
...
* Enhanced +flexmock+ method

  * +flexmock+ will now create both regular mocks and partial mocks
    (+flexstub+ is still included for backwards compatibility).

  * Quick and simple mocks my now be created using an expectation hash
    (example: <tt>flexmock(:foo => 10, :bar => "Hello")</tt> will create a
    mock with two methods, :foo and :bar, defined).

* Refactored implementation, allowing for more granular 
